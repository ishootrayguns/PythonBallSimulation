...
def mouse_click(x,y):
    global simultons, current_object
    if current_object == 'Remove':
        simulton = find(lambda sim : sim.get_location() >= (x-sim.radius,y-sim.radius) and sim.get_location() <= (x+sim.radius,y+sim.radius) )
        for sim in simulton:
            simultons.discard(sim)
    elif current_object != None:
        simultons.add(eval(str(current_object)+'('+str(x)+','+str(y)+')'))
...
def find(p):
    global simultons
    return {sim for sim in simultons if p(sim)}
